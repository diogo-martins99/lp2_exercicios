...
2. O `foreach` esconde a complexidade dos `enumerators` pelo que é preferível usá-lo ao invés de manipular diretamente os `enumerators`.

3. A interaface que a classe implementa é um IEnumerator e trata-se de uma implementação através de outras interfaces intermédias, nomeadamente a IEnumerable.

4. O design pattern é o Iterator pattern.
